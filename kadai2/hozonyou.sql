...
-- いちおうのこす
SELECT * FROM rec_detail AS r_de
JOIN (
    SELECT * FROM (
        SELECT * FROM rec_pay AS rp
        JOIN payment AS py
        ON rp.pay_id = py.pay_id
    ) AS rp_py
    JOIN (
...
ON r_de.r_id = tekito.r_id;


-- receiptをくっつけたrec_detailテーブル
...
-- 「receiptをくっつけたrec_detailテーブル」をくっつけたeat_inテーブル
...
-- うおおおおおおおおおお一応全部
SELECT im_p.p_id, p_name, price, t_id, rate, eat_in_flag, eat_in_rate, d_id, r_id, piece, discount, pay_id, p_amount, pay_name, order_no, r_date, r_time, e_id, regi_no, s_id, resp_no, s_name, s_address, s_tel FROM (
    SELECT p_id, p_name, price, p.t_id, rate FROM product AS p
    JOIN tax AS t
    ON p.t_id = t.t_id
) AS im_p
